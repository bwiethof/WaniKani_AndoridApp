...
data class CollectionMeta(
    ...
    companion object {
        val querys = arrayOf("object", "url", "pages", "total_count", "data_updated_at")
    }
...
data class ResourceMeta(val id: String?, val obj: String?, val url: String?, val dateUpd: String?) {
    companion object {
        val querys = arrayOf("id", "object", "url", "data_updated_at")
    }
}
...
data class Resource<T>(val meta: ResourceMeta, val data: T)

//
// Collection containing Collection meta Data and a Array of Ressources deifned as above
data class Collection<T>(val meta: CollectionMeta?, val data: Array<Resource<T>?>?)
